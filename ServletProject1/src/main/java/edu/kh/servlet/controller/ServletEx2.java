...
public class ServletEx2 extends HttpServlet{
	//Get 방식 요청을 처리하는 메서드(doGet)
	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		
		//전달된 input태그의 값(Parameter) 얻어와 변수에 저장
		String orderer = req.getParameter("orderer");
		//radio버튼은 하나만 선택가능 == 값 1개만 서버로 제출
		String type = req.getParameter("type");
		//select 박스는 1개의 option만 선택 가능== 값 1개만 서버로 제출
		String coffee = req.getParameter("coffee");
		//checkbox와 값이 하나의 name속성으로 여러값이 전달 될 경우
		// req.getParameter() -> 여러 값 중 첫번쨰 값만 얻어옴(String)
		// req.getParameterValues() -> 모든 값을 얻어옴(String[])
		// 							단, 값이 하나도 없으면  null(참조하는 객체가 없다)
		
//		String opt = req.getParameter("option");
		String[] opt = req.getParameterValues("option");
